...
//            signed_bytes = getExtAppSignedData(c, data, padding, saltlen, hashalg, needDigest);
    ...
    public String getSignedData(Context c, String b64data, boolean pkcs1padding) {
        ...
        if (mAuthenticationType == TYPE_EXTERNAL_APP)
            ...
            signed_bytes = getKeyChainSignedDataNew(data, pkcs1padding);
    ...
    public Intent prepareStartService(Context context) {
        Intent intent = getStartServiceIntent(context, null);
...
//        }
    ...
    public Intent prepareStartService(Context context, List<String> excludedApps, int splitMode, String dnsServerIp, boolean allowLanConnection, long timeToShutdownInMillis) {
        Intent intent = getStartServiceIntent(context, null);
        intent.putExtra(OpenVPNService.EXTRA_EXCLUDED_APPS, String.join(",", excludedApps));
        intent.putExtra(OpenVPNService.EXTRA_SPLIT_MODE, splitMode);
        intent.putExtra(OpenVPNService.EXTRA_ALLOW_LAN, allowLanConnection);
        intent.putExtra(OpenVPNService.EXTRA_TIME_TO_SHUTDOWN_IN_MILLIS, timeToShutdownInMillis);
        intent.putExtra(OpenVPNService.EXTRA_DNS_SERVER_IP, dnsServerIp);
        return intent;
    }
...
//    }
    ...
    private byte[] getKeyChainSignedDataNew(byte[] data, boolean pkcs1padding) {
