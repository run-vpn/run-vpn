...
import de.blinkt.openvpn.activities.DisconnectVPNActivity;
...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    public static final String ACTION_PAUSE_VPN = "action_pause";
    ...
    public static final String EXTRA_SPLIT_MODE = "extraSplitMode";
    public static final String EXTRA_ALLOW_LAN = "extraAllowLan";
    public static final String EXTRA_DNS_SERVER_IP = "extra_dns_server_ip";
    public static final String EXTRA_TIME_TO_SHUTDOWN_IN_MILLIS = "extraTimeToShutdown";
    ...
    private boolean allowLanConnection;
    ...
    private String createNotificationChannel(String channelId) {
        if (lastChannel == null || lastChannel.isEmpty()) {
            ...
            Log.d("Some some", getApplicationContext().toString());
        }
    ...
    private void showNotification(final String msg, String tickerText, @NonNull String channel,
        ...
        OpenVpnNotificationBuilder notificationBuilder = Objects.requireNonNull(OpenVPNApikt.INSTANCE.getNotificationBuilder());
        Notification notification = notificationBuilder.buildNotification();
        startForeground(notificationBuilder.getNotificationId(), notification);
    }
    ...
    private void addVpnActionsToNotification(Notification.Builder nbuilder) {
        ...
        if (mDeviceStateReceiver == null || !mDeviceStateReceiver.isUserPaused()) {
            ...
            nbuilder.addAction(R.drawable.ic_stat_vpn_offline,
            ...
            nbuilder.addAction(R.drawable.ic_stat_vpn_outline,
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        String excludedPackages = intent.getStringExtra(EXTRA_EXCLUDED_APPS);
        ...
        timeToShutdown = intent.getLongExtra(EXTRA_TIME_TO_SHUTDOWN_IN_MILLIS, 0L);

        if (excludedPackages != null) {
            Log.d("ExcludedIds", "ids is not null");
            excludedAppIds = Arrays.asList(excludedPackages.split(","));
        } else {
            Log.d("ExcludedIds", "ids is null");
        }

        if (intent != null && intent.getAction() != null) {
            Log.d("ExcludedIds", intent.getAction());
        }
        ...
        if (intent != null && ACTION_PAUSE_VPN.equals(intent.getAction())) {
            try {
                mNotificationAlwaysVisible = true;
                OpenVPNApikt.setStatus(ConnectionStatus.LEVEL_VPNPAUSED, "");
                stopVPN(false);
            } catch (RemoteException e) {
                VpnStatus.logException(e);
            }
    ...
    public ParcelFileDescriptor openTun() {
        ...
        Log.d("OpenVpnService", "openTun: excludedAppIds = " + excludedAppIds + ", mode = " + splitMode);
        ...
        if (excludedAppIds != null) {
            ...
            switch (splitMode) {
                ...
                case 1:
                    excludedAppIds.add(getPackageName());
                    for (String packageId : excludedAppIds) {
                        try {
                ...
                case 2:
                    excludedAppIds.remove(getPackageName());
                    for (String packageId : excludedAppIds) {
                        try {
                    ...
                    break;
                default:
                    throw new IllegalArgumentException("not supported split mode: " + splitMode);
...
//        }
    ...
    public void updateState(String state, String logmessage, int resid, ConnectionStatus level, Intent intent) {
        if (intent != null) {
            OpenVPNApikt.setStatus(level, intent.getAction());
        } else {
            OpenVPNApikt.setStatus(level, null);
        }
...
//                    VpnStatus.getLastCleanLogMessage(this), channel, 0, level, intent);
    ...
    private void setupShutdownTimer() {
        if (timeToShutdown != 0L) {
            timer.schedule(hourlyTask, timeToShutdown);
        }
    }
...
//        showNotification(getString(resid), getString(resid), NOTIFICATION_CHANNEL_NEWSTATUS_ID, 0, LEVEL_WAITING_FOR_USER_INPUT, null);
    ...
    //sending message to main activity
    private void sendMessage(String duration, String lastPacketReceive, String byteIn, String
            byteOut) {
        Intent intent = new Intent("connectionState");
