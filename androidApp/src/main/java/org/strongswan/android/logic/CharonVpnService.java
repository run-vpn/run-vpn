...
public abstract class CharonVpnService extends VpnService implements Runnable, VpnStateService.VpnStateListener {
	...
	*/
    ...
    public void stateChanged() {
        if (mService != null) {
            if (mService.getState() == State.DISCONNECTING) return;
        }
...
//        encodings.add(mCurrentProfile.getProfileCertificate().getEncoded());
    ...
    public class BuilderAdapter {
        ...
        private Builder createBuilder(String name) {
			...
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
				pending = PendingIntent.getActivity(context, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
			} else {
				pending = PendingIntent.getActivity(context, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT);
			}*/
